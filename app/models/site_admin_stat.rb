...
class SiteAdminStat
  ...
  def self.migration_totals(site_token, until_day = Time.utc(2013,10,22))
    totals = where(site_token: site_token, time: { :$lte => until_day }).all.inject(new) do |global_stat, stat|
